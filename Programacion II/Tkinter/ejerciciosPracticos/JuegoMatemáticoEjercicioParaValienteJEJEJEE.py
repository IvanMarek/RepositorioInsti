...
class Aplicacion():
    ...
    def createWidgets(self):
        ...
        self.botonNuevoJuego= ttk.Button(self.window, text= " Nuevo juego ", command= lambda: [self.primerNum.config(state=NORMAL) ,self.primerNum.delete(0,END), self.primerNum.config(state="readonly"), self.segundoNum.config(state=NORMAL) , self.segundoNum.delete(0,END), self.segundoNum.config(state="readonly"),self.numerosAlazar(), self.signosAlAzar(), self.segundoNum.delete(0,END), self.EtiqResultado.delete(0,END),self.EtiqResultado.config(state=NORMAL), self.botonNuevoJuego.config(state=DISABLED), self.botonResultado.config(state=NORMAL)])
        self.botonNuevoJuego.grid(row=3, column=4, sticky=N)
        self.botonNuevoJuego.config(state=DISABLED)


        self.botonSuma= ttk.Radiobutton(self.window, text=" Suma ", variable=n, value=1)
        self.botonSuma.grid(row=3, column=1, sticky=W+E, padx=(20,0), pady=(25,0))
        self.botonSuma.config(state=DISABLED)

        self.botonRestar= ttk.Radiobutton(self.window, text=" Restar ", variable=n, value=2)
        self.botonRestar.grid(row=4, column=1, sticky=W+E, padx=(20,0), pady=(3,0))
        self.botonRestar.config(state=DISABLED)

        self.botonMulti= ttk.Radiobutton(self.window, text=" Multiplicar ", variable=n, value=3)
        self.botonMulti.grid(row=5, column=1, sticky=W+E, padx=(20,0), pady=(3,0))
        self.botonMulti.config(state=DISABLED)

        self.botonDividir= ttk.Radiobutton(self.window, text=" Dividir ", variable=n, value=4)
        ...
        self.EtiqResultado.config(state="readonly")

        self.botonResultado= ttk.Button(self.window, text=" Resultado ", command= lambda: [self.calcular(), self.EtiqResultado.delete(0,END), self.EtiqResultado.config(state="readonly"), self.botonNuevoJuego.config(state=NORMAL), self.botonResultado.config(state=DISABLED)])
        self.botonResultado.grid(row=7, column=4, sticky=W+E)
        self.botonResultado.config(state=DISABLED)
